...
# Test two-image reel generation endpoint in Docker
# Usage: ./docker-test-two-image-reel.sh
...
echo -e "${BLUE}🐳 Docker Test - Two-Image Reel Generation${NC}"
echo "=============================================="
echo ""
...
if [ ! -f "example_two_image_reel_input.json" ]; then
    echo -e "${RED}❌ example_two_image_reel_input.json not found${NC}"
    exit 1
fi

# Test two-image reel generation
echo -e "${BLUE}🎬 Testing POST /generate-two-image-reel...${NC}"
echo ""

OUTPUT_FILE="docker_test_two_image_reel.mp4"
...
if [ -f "$OUTPUT_FILE" ]; then
    ...
    echo -e "${GREEN}✅ Two-image reel generated successfully${NC}"
    echo ""
    echo "   File: $OUTPUT_FILE"
    echo "   Size: $FILE_SIZE"
    
    # Get video duration and info if ffprobe is available
    ...
    echo ""
    echo -e "${GREEN}🎉 Two-image reel generation test passed!${NC}"
    echo ""
