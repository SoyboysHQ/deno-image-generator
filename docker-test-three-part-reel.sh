...
# Docker test script - Three-Part Reel Generation Endpoint
...
echo -e "${BLUE}🐳 Docker Test - Three-Part Reel Generation${NC}"
echo "=============================================="
echo ""
...
if [ ! -f "example_three_part_reel_input.json" ]; then
    echo -e "${RED}❌ example_three_part_reel_input.json not found${NC}"
    exit 1
fi

# Test three-part reel generation
echo -e "${BLUE}🎬 Testing POST /generate-three-part-reel...${NC}"
echo ""

OUTPUT_FILE="docker_test_three_part_reel.mp4"
...
if [ -f "$OUTPUT_FILE" ]; then
    ...
    echo -e "${GREEN}✅ Three-part reel generated successfully${NC}"
    echo ""
    ...
    echo ""
    echo -e "${GREEN}🎉 Three-part reel generation test passed!${NC}"
    echo ""
