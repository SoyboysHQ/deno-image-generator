...
async function generateCoverSlide(
  ...
  // Draw swipe icon in bottom right corner (doubled size)
  const currentDir = Deno.cwd();
  const swipeIconPath = join(currentDir, 'assets', 'icons', 'swipe.svg');
  let swipeIcon: any = null;
  try {
    swipeIcon = await loadImage(swipeIconPath);
  } catch (error) {
    console.error(`Failed to load swipe icon: ${error}`);
  }
  
  if (swipeIcon) {
    const iconSize = 120; // 50% bigger (80px * 1.5)
    // Calculate icon dimensions maintaining aspect ratio
    const iconAspect = swipeIcon.width / swipeIcon.height;
    const iconDrawWidth = iconSize * iconAspect; // Maintain aspect ratio
    const iconDrawHeight = iconSize;
    // Position in bottom right corner
    const iconX = WIDTH - iconDrawWidth - 40; // 40px from right edge
    const iconY = HEIGHT - iconDrawHeight - 40; // 40px from bottom edge
    ctx.drawImage(swipeIcon, iconX, iconY, iconDrawWidth, iconDrawHeight);
  }
  
  // Draw white rounded rectangle with "Key Takeaways" at center bottom
  const text = coverText || 'Key Takeaways';
  const textFont = 'bold 48px Arial'; // Arial bold
  ctx.font = textFont;
  const textWidth = ctx.measureText(text).width;
  
  const padding = 30;
  const borderRadius = 40; // Increased border radius
  const rectWidth = textWidth + padding * 2;
  const rectHeight = 60 + padding * 2;
  const rectX = (WIDTH - rectWidth) / 2; // Center horizontally
  const rectY = HEIGHT - rectHeight - 80; // Moved up (80px from bottom instead of 40px)
  
  // Draw soft drop shadow for white rounded rectangle (similar to book cover)
  ...
  for (const layer of shadowLayers) {
    ...
    // Create shadow path with rounded corners
    ctx.beginPath();
    ctx.moveTo(rectX + layer.offset + borderRadius, rectY + layer.offset);
    ctx.lineTo(rectX + rectWidth - borderRadius + layer.offset, rectY + layer.offset);
    ctx.quadraticCurveTo(
      rectX + rectWidth + layer.offset,
      rectY + layer.offset,
      rectX + rectWidth + layer.offset,
      rectY + borderRadius + layer.offset
    );
    ctx.lineTo(rectX + rectWidth + layer.offset, rectY + rectHeight - borderRadius + layer.offset);
    ctx.quadraticCurveTo(
      rectX + rectWidth + layer.offset,
      rectY + rectHeight + layer.offset,
      rectX + rectWidth - borderRadius + layer.offset,
      rectY + rectHeight + layer.offset
    );
    ctx.lineTo(rectX + borderRadius + layer.offset, rectY + rectHeight + layer.offset);
    ctx.quadraticCurveTo(
      rectX + layer.offset,
      rectY + rectHeight + layer.offset,
      rectX + layer.offset,
      rectY + rectHeight - borderRadius + layer.offset
    );
    ctx.lineTo(rectX + layer.offset, rectY + borderRadius + layer.offset);
    ctx.quadraticCurveTo(
      rectX + layer.offset,
      rectY + layer.offset,
      rectX + borderRadius + layer.offset,
      rectY + layer.offset
    );
    ctx.closePath();
    ctx.fill();
  ...
  // Draw text - vertically centered and more bold with letter spacing
  const textX = rectX + padding;
  // Calculate vertical center: middle of rectangle accounting for text baseline
  const metrics = ctx.measureText(text);
  const textHeight = metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;
  const textY = rectY + (rectHeight + textHeight) / 2 - metrics.actualBoundingBoxDescent; // Vertically centered
  // Use heavier font weight for more bold appearance
  ctx.font = '900 48px Arial'; // 900 is the heaviest weight (more bold than "bold" which is 700)
  // Add 2% letter spacing (48px * 0.02 = 0.96px, rounded to 1px)
