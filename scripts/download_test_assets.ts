// Download sample assets for testing

console.log("📥 Downloading test assets...\n");

// Create a simple solid color background image (1x1 pixel, will be stretched)
// This is a minimal JPEG file (solid purple color)
console.log("🎨 Creating test background image...");
const jpegData = new Uint8Array([
  
]);

// Create a proper 1080x1350 purple gradient image using ImageMagick-like approach
// Since we can't use canvas, let's just copy a valid JPEG
console.log("   Creating a simple gradient background...");

// For now, let's create a script to generate with ImageMagick if available
// or provide instructions
const hasMagick = await Deno.run({
  cmd: ["which", "convert"],
  stdout: "null",
  stderr: "null",
}).status();

if (hasMagick.success) {
  console.log("   Using ImageMagick to create background...");
  const p = Deno.run({
    cmd: [
      "convert",
      "-size",
      "1080x1350",
      "gradient:#667eea-#764ba2",
      "assets/images/background.jpeg",
    ],
    stdout: "inherit",
    stderr: "inherit",
  });
  await p.status();
  console.log("✅ Created assets/images/background.jpeg with gradient\n");
} else {
  console.log("   ⚠️  ImageMagick not found, creating placeholder...");
  // Create a simple placeholder file
  await Deno.writeFile("assets/images/background.jpeg", jpegData);
  console.log("✅ Created assets/images/background.jpeg (placeholder)\n");
  console.log("   Note: For better results, replace with a 1080x1350px image\n");
}

// Download Merriweather fonts from Google Fonts GitHub repo
console.log("🔤 Downloading Merriweather fonts...");

const fontUrls = [
  {
    name: "assets/fonts/Merriweather-Regular.ttf",
    url: "https://github.com/SorkinType/Merriweather/raw/master/fonts/ttf/Merriweather-Regular.ttf",
  },
  {
    name: "assets/fonts/Merriweather-Bold.ttf",
    url: "https://github.com/SorkinType/Merriweather/raw/master/fonts/ttf/Merriweather-Bold.ttf",
  },
  {
    name: "assets/fonts/Merriweather-Italic.ttf",
    url: "https://github.com/SorkinType/Merriweather/raw/master/fonts/ttf/Merriweather-Italic.ttf",
  },
];

let allFontsDownloaded = true;

for (const font of fontUrls) {
  try {
    console.log(`   Downloading ${font.name}...`);
    const response = await fetch(font.url);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}`);
    }
    const fontData = await response.arrayBuffer();
    await Deno.writeFile(font.name, new Uint8Array(fontData));
    const size = ((fontData.byteLength) / 1024).toFixed(2);
    console.log(`   ✅ ${font.name} downloaded (${size} KB)`);
  } catch (error) {
    console.log(`   ❌ Failed to download ${font.name}: ${error.message}`);
    allFontsDownloaded = false;
  }
}

if (!allFontsDownloaded) {
  console.log("\n⚠️  Some fonts failed to download.");
  console.log("   You can download them manually from:");
  console.log("   https://fonts.google.com/specimen/Merriweather\n");
}

console.log("\n" + "=".repeat(50));
console.log("🎉 Setup complete!\n");
console.log("Run this to verify:");
console.log("   deno task test-setup\n");
console.log("Then generate your image:");
console.log('   deno task generate "$(cat tests/fixtures/example_input.json)"');
