...
# Docker test script - Book Takeaways Carousel Generation Endpoint
...
echo -e "${BLUE}🐳 Docker Test - Book Takeaways Carousel Generation${NC}"
echo "========================================================"
echo ""
...
if [ ! -f "example_book_takeaways_carousel_input.json" ]; then
    echo -e "${RED}❌ example_book_takeaways_carousel_input.json not found${NC}"
    exit 1
fi

# Test book takeaways carousel generation
echo -e "${BLUE}📚 Testing POST /generate-book-takeaways-carousel...${NC}"
echo ""

OUTPUT_FILE="docker_test_book_takeaways_carousel_response.zip"
EXTRACT_DIR="docker_test_book_takeaways_carousel_slides"

HTTP_STATUS=$(curl -X POST http://localhost:8000/generate-book-takeaways-carousel \
    -H "Content-Type: application/json" \
    -d @example_book_takeaways_carousel_input.json \
    --output "$OUTPUT_FILE" \
    --max-time 300 \
    -s -w "%{http_code}")
...
if [ -f "$OUTPUT_FILE" ]; then
    ...
    if file "$OUTPUT_FILE" | grep -q "Zip archive"; then
        echo -e "${GREEN}✅ Book takeaways carousel ZIP generated successfully${NC}"
        echo ""
        ...
        if command -v unzip &> /dev/null; then
            ...
            # Don't automatically open first slide
    ...
    echo ""
    echo -e "${GREEN}🎉 Book takeaways carousel generation test passed!${NC}"
else
